...
**Prebuilt binary** — via the install script, once the repo is public:

```bash
curl -fsSL https://raw.githubusercontent.com/<owner>/sandbar/main/install.sh | SANDBAR_OWNER=<owner> bash
```
...
installs the binary as `sandbar` (module root is the repo root).
...
- **Client prefs** — `~/.config/sandbar/client.yaml` holds TUI-only
  ...
  auto-created with commented defaults on first run.
...
| `/search` | Full-text search past conversations |
...
```
src/cmd/cli/          REPL entry point
src/internal/agent/   reasoning loop, subagents, steering
src/internal/backend/ Backend seam (LocalBackend)
src/internal/llm/     OpenAI-compatible client, stream events, tokens
src/internal/tools/   tools, registry, approvals, jobs, ssh
src/internal/memory/  SQLite store, FTS5 search, compression records
src/internal/persona/ system prompt assembly, skills
src/internal/config/  YAML config, resolution, zero-config boot
src/internal/cliui/   shared CLI rendering
src/internal/ui/theme/ palettes
